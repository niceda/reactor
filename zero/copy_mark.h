...
event_t *_get_event_t(reactor_t *r) {
    r->iter++;
    while (r->events[MAX_CONN & r->iter].fd > 0) {
        r->iter++;
    }
...
int event_buffer_read(event_t *e) {
    ...
    while (1) {
        ...
        if (n == 0) {
            ...
            if (errno == EINTR)// EINTR代表“Interrupted system call”，表示系统调用被信号中断。当进程在进行非阻塞I/O操作时，如果收到一个信号，就会返回这个错误码。此时，进程可以重新发起I/O请求或者处理其他任务。
                continue;
            if (errno == EWOULDBLOCK)// EWOULDBLOCK代表“Operation would block”，表示操作将会阻塞。当进程在进行非阻塞I/O操作时，如果该操作无法立即完成（例如读取缓冲区为空），就会返回这个错误码。此时，进程可以等待一段时间后再次尝试操作或者执行其他任务。EWOULDBLOCK代表“Operation would block”，表示操作将会阻塞。当进程在进行非阻塞I/O操作时，如果该操作无法立即完成（例如读取缓冲区为空），就会返回这个错误码。此时，进程可以等待一段时间后再次尝试操作或者执行其他任务。
                break;
            printf("erad error fd = %d err = %s\n", fd, strerror(errno));
            if (e->error_fn)
