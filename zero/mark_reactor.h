...
event_t *new_event(reactor_t *R, int fd, event_callback_fn rd,
                   event_callback_fn wt, error_callback_fn err) {
...
int create_server(reactor_t *R, short port, event_callback_fn func) {
    ...
    int reuse = 1;
    if (setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, (void *)&reuse,// SO_REUSEADDR: 避免time_wait时间，端口复用
                   sizeof(int)) == -1) {
        printf("reuse address error: %s\n", strerror(errno));
        return -1;
    }

    if (bind(listenfd, (struct sockaddr *)&addr, sizeof(struct sockaddr_in)) <
        0) {
        printf("bind error %s\n", strerror(errno));
        return -1;
    }
...
int event_buffer_write(event_t *e, void *buf, int sz) { // todo
    buffer_t *r = &e->out;
    if (buffer_len(r) == 0) {
